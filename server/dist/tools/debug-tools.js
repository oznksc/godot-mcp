...
export function registerDebugTools(server, bridge) {
    server.tool('debug_get_output', 'Get the console/output log', {
    ...
    server.tool('debug_get_errors', 'Get runtime errors', {
    ...
    server.tool('debug_get_warnings', 'Get runtime warnings', {
    ...
    server.tool('debug_clear_console', 'Clear the console output', {}, async () => {
        const result = await bridge.sendCommand('debug_clear_console');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('debug_get_scene_tree', 'Get the scene tree of the running project', {}, async () => {
