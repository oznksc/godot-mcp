...
export function registerNodeTools(server, bridge) {
    server.tool('node_add', 'Add a new node to the scene tree', {
    ...
    server.tool('node_move', 'Move a node to a new parent', {
    ...
    server.tool('node_duplicate', 'Duplicate a node', {
    ...
    server.tool('node_get_properties', 'Get all properties of a node', {
        path: z.string().describe('Node path'),
    }, async ({ path }) => {
        const result = await bridge.sendCommand('node_get_properties', { path });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('node_set_property', 'Set a property on a node', {
        path: z.string().describe('Node path'),
    ...
    server.tool('node_set_properties', 'Set multiple properties on a node at once', {
        path: z.string().describe('Node path'),
    ...
    server.tool('node_find', 'Find a node by name in the scene tree', {
    ...
    server.tool('node_find_by_type', 'Find all nodes of a given type', {
        type: z.string().describe('Node type to search for'),
    }, async ({ type }) => {
        const result = await bridge.sendCommand('node_find_by_type', { type });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('node_get_children', 'Get children of a node', {
        path: z.string().describe('Node path'),
    ...
    server.tool('node_get_parent', 'Get the parent of a node', {
        path: z.string().describe('Node path'),
    }, async ({ path }) => {
        const result = await bridge.sendCommand('node_get_parent', { path });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('node_set_groups', 'Set groups for a node', {
        path: z.string().describe('Node path'),
    ...
    server.tool('node_get_groups', 'Get groups of a node', {
        path: z.string().describe('Node path'),
    }, async ({ path }) => {
        const result = await bridge.sendCommand('node_get_groups', { path });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('node_set_meta', 'Set metadata on a node', {
        path: z.string().describe('Node path'),
    ...
    server.tool('node_get_meta', 'Get metadata from a node', {
        path: z.string().describe('Node path'),
