import { z } from 'zod';
export function registerLightingTools(server, bridge) {
    server.tool('lighting_create', 'Create a light node (2D or 3D)', {
    ...
    server.tool('lighting_set_properties', 'Set properties on a light node', {
    ...
    server.tool('lighting_create_environment', 'Create an Environment resource', {
    ...
    server.tool('lighting_configure_world', 'Configure WorldEnvironment on a node', {
