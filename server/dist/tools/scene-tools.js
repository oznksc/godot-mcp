...
export function registerSceneTools(server, bridge) {
    server.tool('scene_create', 'Create a new Godot scene with a root node', {
    ...
    server.tool('scene_read', 'Read the current scene structure as JSON', {}, async () => {
        const result = await bridge.sendCommand('scene_read');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('scene_save', 'Save the current scene', {
    ...
    server.tool('scene_list', 'List all scenes in the project', {}, async () => {
        const result = await bridge.sendCommand('scene_list');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('scene_open', 'Open a scene in the Godot editor', {
        path: z.string().describe('Scene path (res://...)'),
    }, async ({ path }) => {
        const result = await bridge.sendCommand('scene_open', { path });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('scene_close', 'Close the current scene in the editor', {}, async () => {
        const result = await bridge.sendCommand('scene_close');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('scene_get_current', 'Get the currently open scene', {}, async () => {
        const result = await bridge.sendCommand('scene_get_current');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('scene_instance', 'Instance a scene as a child in the current scene', {
    ...
    server.tool('scene_get_hierarchy', 'Get the full scene hierarchy as a tree structure', {}, async () => {
        const result = await bridge.sendCommand('scene_get_hierarchy');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('scene_get_open_scenes', 'List all open scene tabs in the editor', {}, async () => {
        const result = await bridge.sendCommand('scene_get_open_scenes');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('scene_export_mesh_library', 'Export the current scene as a MeshLibrary for GridMap', {
