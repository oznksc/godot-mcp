...
export function registerSignalTools(server, bridge) {
    server.tool('signal_connect', 'Connect a signal to a method on a target node', {
    ...
    server.tool('signal_disconnect', 'Disconnect a signal from a method', {
        source_path: z.string().describe('Source node path'),
        signal_name: z.string().describe('Signal name'),
        target_path: z.string().describe('Target node path'),
    ...
    server.tool('signal_list_connections', 'List all signal connections for a node', {
        path: z.string().describe('Node path'),
    }, async ({ path }) => {
        const result = await bridge.sendCommand('signal_list_connections', { path });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('signal_list_available', 'List all available signals on a node', {
        path: z.string().describe('Node path'),
    }, async ({ path }) => {
        const result = await bridge.sendCommand('signal_list_available', { path });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('signal_emit', 'Emit a signal on a node', {
        path: z.string().describe('Node path'),
        signal_name: z.string().describe('Signal name'),
...
//# sourceMappingURL=signal-tools.js.map