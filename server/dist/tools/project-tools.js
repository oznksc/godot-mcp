...
export function registerProjectTools(server, bridge) {
    server.tool('project_get_info', 'Get project information (name, version, structure)', {}, async () => {
        const result = await bridge.sendCommand('project_get_info');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('project_get_settings', 'Get all project settings', {}, async () => {
        const result = await bridge.sendCommand('project_get_settings');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('project_update_setting', 'Update a project setting', {
    ...
    server.tool('project_get_input_map', 'Get the project input map', {}, async () => {
        const result = await bridge.sendCommand('project_get_input_map');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('project_configure_input_map', 'Add or update an input action', {
    ...
    server.tool('project_get_collision_layers', 'Get collision layer and mask names', {}, async () => {
        const result = await bridge.sendCommand('project_get_collision_layers');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('project_setup_autoload', 'Add an autoload singleton', {
    ...
    server.tool('project_remove_autoload', 'Remove an autoload singleton', {
        name: z.string().describe('Autoload name to remove'),
    }, async ({ name }) => {
        const result = await bridge.sendCommand('project_remove_autoload', { name });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('project_get_class_list', 'Get the list of registered global classes', {}, async () => {
        const result = await bridge.sendCommand('project_get_class_list');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('project_get_export_presets', 'Get export presets', {}, async () => {
