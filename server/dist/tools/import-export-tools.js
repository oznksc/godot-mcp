...
export function registerImportExportTools(server, bridge) {
    server.tool('import_reimport', 'Reimport files in the project', {
    ...
    server.tool('import_get_settings', 'Get import settings for a file', {
        path: z.string().describe('File path (res://...)'),
    }, async ({ path }) => {
        const result = await bridge.sendCommand('import_get_settings', { path });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('import_set_settings', 'Update import settings for a file', {
        path: z.string().describe('File path (res://...)'),
    ...
    server.tool('export_run', 'Run export with a preset', {
    ...
    server.tool('export_get_presets', 'List all export presets', {}, async () => {
        const result = await bridge.sendCommand('export_get_presets');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
}
//# sourceMappingURL=import-export-tools.js.map