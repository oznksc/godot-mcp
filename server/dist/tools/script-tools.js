...
export function registerScriptTools(server, bridge) {
    server.tool('script_create', 'Create a new GDScript file', {
    ...
    server.tool('script_read', 'Read the content of a script file', {
        path: z.string().describe('Script path (res://...)'),
    }, async ({ path }) => {
        const result = await bridge.sendCommand('script_read', { path });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('script_write', 'Write content to a script file', {
        path: z.string().describe('Script path (res://...)'),
    ...
    server.tool('script_attach', 'Attach a script to a node', {
    ...
    server.tool('script_detach', 'Detach the script from a node', {
        node_path: z.string().describe('Node path in the scene'),
    }, async ({ node_path }) => {
        const result = await bridge.sendCommand('script_detach', { node_path });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('script_validate', 'Validate GDScript syntax without saving', {
        content: z.string().describe('GDScript code to validate'),
    }, async ({ content }) => {
        const result = await bridge.sendCommand('script_validate', { content });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('script_list', 'List all GDScript files in the project', {}, async () => {
