import { z } from 'zod';
export function registerFileTools(server, bridge) {
    server.tool('file_list_dir', 'List files in a project directory', {
        path: z.string().default('res://').describe('Directory path (res://...)'),
    }, async ({ path }) => {
        const result = await bridge.sendCommand('file_list_dir', { path });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('file_read', 'Read a file from the project', {
        path: z.string().describe('File path (res://...)'),
    }, async ({ path }) => {
        const result = await bridge.sendCommand('file_read', { path });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('file_write', 'Write content to a file in the project', {
        path: z.string().describe('File path (res://...)'),
    ...
    server.tool('file_delete', 'Delete a file from the project', {
        path: z.string().describe('File path (res://...)'),
    }, async ({ path }) => {
        const result = await bridge.sendCommand('file_delete', { path });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('file_rename', 'Rename a file in the project', {
    ...
    server.tool('file_search', 'Search for files in the project by name or content', {
