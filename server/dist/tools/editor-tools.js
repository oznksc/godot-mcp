...
export function registerEditorTools(server, bridge) {
    server.tool('editor_get_state', 'Get the current editor state (open scenes, active screen, playing status)', {}, async () => {
        const result = await bridge.sendCommand('editor_get_state');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('editor_set_main_screen', 'Switch the main editor screen', {
    ...
    server.tool('editor_rescan_filesystem', 'Rescan the project filesystem', {}, async () => {
        const result = await bridge.sendCommand('editor_rescan_filesystem');
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('editor_restart', 'Restart the Godot editor', {
