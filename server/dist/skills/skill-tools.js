import { z } from 'zod';
import { getSkillContent, getMatchedSkillsContext, listAvailableSkills } from './skill-router.js';
export function registerSkillTools(server) {
    server.tool('skill_query', 'Query Godot knowledge skills — finds relevant skill modules based on a natural language question about Godot development', {
        ...
        if (!context) {
            return {
                content: [{ type: 'text', text: `No relevant skills found for: "${query}"\n\nAvailable skills: ${listAvailableSkills().map(s => s.name).join(', ')}` }],
            };
        }
        return { content: [{ type: 'text', text: context }] };
    });
    server.tool('skill_get', 'Get a specific Godot skill module by ID', {
        skill_id: z.enum([
    ...
    server.tool('skill_list', 'List all available Godot skill modules', {}, async () => {
        const skills = listAvailableSkills();
        const list = skills.map(s => `- **${s.id}** — ${s.name}: ${s.description}`).join('\n');
        return { content: [{ type: 'text', text: `# Available Godot Skills\n\n${list}` }] };
    });
}
