...
export function registerPhysicsTools(server: McpServer, bridge: GodotBridge): void {
  server.tool(
    'physics_set_collision_shape',
    'Set the collision shape for a physics node',
    {
  ...
  server.tool(
    'physics_set_collision_layer',
    'Set collision layer and mask for a node',
    {
  ...
  server.tool(
    'physics_configure_material',
    'Configure PhysicsMaterial for a physics node',
    {
      path: z.string().describe('Node path'),
  ...
  server.tool(
    'physics_get_body_state',
    'Get physics body state at runtime',
    {
