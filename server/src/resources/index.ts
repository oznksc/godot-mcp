...
export function registerResources(server: McpServer, bridge: GodotBridge): void {
  server.resource(
    'project-info',
    'godot://project/info',
    { description: 'Project metadata (name, version, structure)', mimeType: 'application/json' },
    async () => {
  ...
  server.resource(
    ...
    { description: 'All project settings', mimeType: 'application/json' },
    async () => {
  ...
  server.resource(
    'current-scene',
    'godot://scene/current',
    { description: 'Currently open scene structure', mimeType: 'application/json' },
    async () => {
  ...
  server.resource(
    'editor-state',
    'godot://editor/state',
    { description: 'Current editor state (open scenes, active screen, playing status)', mimeType: 'application/json' },
    async () => {
  ...
  server.resource(
    ...
    { description: 'Console and debug output', mimeType: 'text/plain' },
    async () => {
  ...
  server.resource(
    ...
    { description: 'Runtime execution status', mimeType: 'application/json' },
    async () => {
  ...
  server.resource(
    'classdb',
    new ResourceTemplate('godot://classdb/{className}', {
      list: async () => ({
        resources: [{ uri: 'godot://classdb/Node2D', name: 'Node2D' }],
      }),
    }),
    { description: 'Query Godot ClassDB for class information', mimeType: 'application/json' },
    async (uri, { className }) => {
