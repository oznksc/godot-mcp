...
let registryCache: Repository[] | null = null;

function loadRegistry(): Repository[] {
...
function calculateScore(query: string, type: string, example: ExampleEntry): number {
...
export function getExampleContent(repoUrl: string, examplePath: string): string {
...
export function getExamplesByDifficulty(difficulty: string): ExampleMatch[] {
  const repos = loadRegistry();
  const matches: ExampleMatch[] = [];
  for (const repo of repos) {
    for (const example of repo.examples) {
...
export function getExamplesByType(type: string): ExampleMatch[] {
  const repos = loadRegistry();
  const matches: ExampleMatch[] = [];
  for (const repo of repos) {
    for (const example of repo.examples) {
